...
public class FileHandleEmu {
	...
	public FileHandleEmu[] list () {
		if (type == FileType.Classpath) throw new GdxRuntimeException("Cannot list a classpath directory: " + file);
		String[] relativePaths = file().list();
		if (relativePaths == null) return new FileHandleEmu[0];
		FileHandleEmu[] handles = new FileHandleEmu[relativePaths.length];
	 ...
	 * @throws GdxRuntimeException if this file is an {@link FileType#Classpath} file. */
	public FileHandleEmu[] list (FileFilter filter) {
		if (type == FileType.Classpath) throw new GdxRuntimeException("Cannot list a classpath directory: " + file);
		File file = file();
		String[] relativePaths = file.list();
		if (relativePaths == null) return new FileHandleEmu[0];
		...
		for (int i = 0, n = relativePaths.length; i < n; i++) {
			...
			FileHandleEmu child = child(path);
			if (!filter.accept(child.file())) continue;
			handles[count] = child;
			count++;
		}
		if (count < relativePaths.length) {
			FileHandleEmu[] newHandles = new FileHandleEmu[count];
	 ...
	 * @throws GdxRuntimeException if this file is an {@link FileType#Classpath} file. */
	public FileHandleEmu[] list (FilenameFilter filter) {
		if (type == FileType.Classpath) throw new GdxRuntimeException("Cannot list a classpath directory: " + file);
		File file = file();
		String[] relativePaths = file.list();
		if (relativePaths == null) return new FileHandleEmu[0];
	 ...
	 * @throws GdxRuntimeException if this file is an {@link FileType#Classpath} file. */
	public FileHandleEmu[] list (String suffix) {
		if (type == FileType.Classpath) throw new GdxRuntimeException("Cannot list a classpath directory: " + file);
		String[] relativePaths = file().list();
		if (relativePaths == null) return new FileHandleEmu[0];
		FileHandleEmu[] handles = new FileHandleEmu[relativePaths.length];
	...
	public FileHandleEmu child (String name) {
		if (file.getPath().length() == 0) return new FileHandleEmu(new File(name), type);
		return new FileHandleEmu(new File(file, name), type);
	}

	/** Returns a handle to the sibling with the specified name.
	 * @throws GdxRuntimeException if this file is the root. */
	public FileHandleEmu sibling (String name) {
		if (file.getPath().length() == 0) throw new GdxRuntimeException("Cannot get the sibling of the root.");
		return new FileHandleEmu(new File(file.getParent(), name), type);
	}

	public FileHandleEmu parent () {
		...
		return new FileHandleEmu(parent, type);
	}
	...
	public void copyTo (FileHandleEmu dest) {
		if (!isDirectory()) {
	...
	public void moveTo (FileHandleEmu dest) {
		switch (type) {
		case Classpath:
	...
	public boolean equals (Object obj) {
		if (!(obj instanceof FileHandleEmu)) return false;
		FileHandleEmu other = (FileHandleEmu)obj;
	...
	static public FileHandleEmu tempFile (String prefix) {
		try {
			return new FileHandleEmu(File.createTempFile(prefix, null));
		} catch (IOException ex) {
			throw new GdxRuntimeException("Unable to create temp file.", ex);
		}
	}

	static public FileHandleEmu tempDirectory (String prefix) {
		try {
	...
	static private void copyFile (FileHandleEmu source, FileHandleEmu dest) {
		try {
	...
	static private void copyDirectory (FileHandleEmu sourceDir, FileHandleEmu destDir) {
		destDir.mkdirs();
		FileHandleEmu[] files = sourceDir.list();
		for (int i = 0, n = files.length; i < n; i++) {
			FileHandleEmu srcFile = files[i];
			FileHandleEmu destFile = destDir.child(srcFile.name());
