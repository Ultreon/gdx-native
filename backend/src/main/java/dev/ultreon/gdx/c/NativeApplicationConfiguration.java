...
import dev.ultreon.gdx.c.NativeGraphics.NativeMonitor;
import dev.ultreon.gdx.c.glfw.GLFW;
import dev.ultreon.gdx.c.glfw.GLFW.GLFWVidMode;

import java.io.PrintStream;

public class NativeApplicationConfiguration extends NativeWindowConfiguration {
    ...
    static NativeApplicationConfiguration copy(NativeApplicationConfiguration config) {
        NativeApplicationConfiguration copy = new NativeApplicationConfiguration();
        copy.set(config);
        return copy;
    }

    void set(NativeApplicationConfiguration config) {
     ...
     * <p>
     * Use {@link NativeApplication#setGLDebugMessageControl(NativeApplication.GLDebugMessageSeverity, boolean)} to enable or
    ...
    public static DisplayMode getDisplayMode() {
        NativeApplication.initializeGlfw();
        GLFWVidMode videoMode = GLFW.getVideoMode(GLFW.getPrimaryMonitor());
        return new NativeGraphics.NativeDisplayMode(GLFW.getPrimaryMonitor(), videoMode.width, videoMode.height,
    ...
    public static DisplayMode getDisplayMode(Monitor monitor) {
        NativeApplication.initializeGlfw();
        GLFWVidMode videoMode = GLFW.getVideoMode(((NativeMonitor) monitor).monitorHandle);
        return new NativeGraphics.NativeDisplayMode(((NativeMonitor) monitor).monitorHandle, videoMode.width, videoMode.height,
                videoMode.refreshRate, videoMode.redBits + videoMode.greenBits + videoMode.blueBits);
    }
    ...
    public static DisplayMode[] getDisplayModes() {
        NativeApplication.initializeGlfw();
        GLFWVidMode[] videoModes = GLFW.getVideoModes(GLFW.getPrimaryMonitor());
        DisplayMode[] result = new DisplayMode[videoModes.length];
        for (int i = 0; i < result.length; i++) {
            GLFWVidMode videoMode = videoModes[i];
            result[i] = new NativeGraphics.NativeDisplayMode(GLFW.getPrimaryMonitor(), videoMode.width, videoMode.height,
                    videoMode.refreshRate, videoMode.redBits + videoMode.greenBits + videoMode.blueBits);
        }
        return result;
    }
    ...
    public static DisplayMode[] getDisplayModes(Monitor monitor) {
        NativeApplication.initializeGlfw();
        GLFWVidMode[] videoModes = GLFW.getVideoModes(((NativeMonitor) monitor).monitorHandle);
        DisplayMode[] result = new DisplayMode[videoModes.length];
        for (int i = 0; i < result.length; i++) {
            GLFWVidMode videoMode = videoModes[i];
            result[i] = new NativeGraphics.NativeDisplayMode(((NativeMonitor) monitor).monitorHandle, videoMode.width,
    ...
    static NativeMonitor toNativeMonitor(long glfwMonitor) {
        int[] tmp = new int[1];
        int[] tmp2 = new int[1];
        ...
        return new NativeMonitor(glfwMonitor, virtualX, virtualY, name);
    }

    static GridPoint2 calculateCenteredWindowPosition(NativeMonitor monitor, int newWidth, int newHeight) {
        int[] tmp = new int[1];
        int[] tmp2 = new int[1];
        int[] tmp3 = new int[1];
        int[] tmp4 = new int[1];
